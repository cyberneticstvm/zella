...
class PDFController extends Controller
{
    ...
    public function salesinvoice($id){
        ...
        $sales = DB::table('sales_details as s')->leftJoin('products as p', 'p.id', '=', 's.product')->select('s.id', 's.product', 's.qty', 's.price', 's.total', 's.vat_percentage', 's.old_product', 's.is_return', 's.type', 'p.name', 'p.vat_applicable', DB::raw("DATE_FORMAT(s.return_date, '%d/%b/%Y') AS return_date"))->where('s.sales_id', $id)->get();      
