...
class PurchaseExport implements FromCollection, WithHeadings, WithPreCalculateFormulas, WithStyles
{
    /**
    ...
    */
    ...
    public function __construct($request){
        ...
        $this->purchases = DB::table('purchases as p')->leftJoin('purchase_details as pd', 'p.id', 'pd.purchase_id')->leftJoin('suppliers as s', 'p.supplier', '=', 's.id')->selectRaw("p.id, p.invoice_number, s.name as sname, DATE_FORMAT(p.order_date, '%d/%b/%Y') as odate, DATE_FORMAT(p.delivery_date, '%d/%b/%Y') as ddate, p.payment_mode, SUM(pd.total)+p.other_expense as total")->where('pd.is_return', 0)->whereBetween('p.delivery_date', [$from, $to])->when(isset($supplier), function($query) use ($request, $supplier){
